...
def parse_method(node: Element) -> dict[str]:
  ...
  result["description"] = node.find("description").text.strip() if node.text is not None else ''
...
def parse_class(root: Element, docs: list[ElementTree]) -> dict[str]:
  ...
  for node in root:
    match node.tag:
      case "members": result["properties"] = parse_properties(node); continue
      case "methods": result["methods"] = parse_methods(node); continue
      case "signals": result["signals"] = parse_signals(node); continue
      case "constants":
